...
class SizeBias:
    ...
    def __init__(self, gal_pos, void_pos, void_radii, void_delta, rho_mean, volume, Rmax=5):
    ...
    def fit_profile(self, nbins=500, rbins=5, rescale=True):
        r"""
        Fit the rescaled (r / R) void density profile (in units of rho / rho_mean)

        Parameters
        ----------
        nbins: int, default=500
        ...
        tree = cKDTree((self.gal_pos + boxshift) % boxsize, compact_nodes=False, 
                       balanced_tree=False, boxsize=boxsize)

        N = np.zeros((self.r.size - 1, self.void_radii.size))
        for (i,r) in enumerate(self.r[1:]):
            R = r * self.void_radii
            N[i] = tree.query_ball_point(self.void_pos + boxshift, R,  
                                           workers=-1, return_length=True)

        V = 4 * np.pi * (self.r[1:,None] * self.void_radii)**3 / 3

        self.r = self.r[None, 1:]
        ...
        profile = np.insert(self.profile, [0, self.profile.shape[1], self.profile.shape[1]], [0, 1, 1], axis=1)
        self.splines = [
                    Interpolate(r[i if self.r.shape[0] > 1 else 0], profile[i], extrapolate=True) 
                    for i in range(self.profile.shape[0])
                    ]

    def interpolate(self, r, R):
        r"""
        Interpolate the enclosed density profile.

        Parameters
        ----------
        r: array (N)
            Array of radii points along the profile to interpolate.
        R: float, array(N)
            Value (or array) of the true void radius.
        """
        R = np.atleast_1d(R)

        fit = np.zeros((r.size, R.size))
    ...
    def plot_profile(self, rho_enclosed=False, ax=None, save=None, **kwargs):
        show = False
        if ax is None: 
        ...
        if rho_enclosed:
            rho = lambda r, rho_enc : rho_enc
            ylabel = r"$\rho(<r) / \bar{\rho}$"
        else:
            # convert enclosed density to 1D profile
            rho = lambda r, rho_enc : rho_enc + np.gradient(rho_enc, r) * r / 3
            ylabel = r"$\rho(r) / \bar{\rho}$"

        rr = np.linspace(0, self.Rmax, 100)
        for (i, R) in enumerate(self.rsplit):
            # plot measured profile
            r = self.r[i] if self.r.shape[0] > 1 else self.r[0]
            ax.plot(r, rho(r, self.profile[i]), 
                    label=rf"$\langle R_v \rangle = {R:.1f}$")
            # plot interpolated profile
            rho_interp = self.interpolate(rr * R, R)[:,0]
            ax.plot(rr, rho(rr, rho_interp), c='k', ls='--', lw=0.8)
    ...
    def _F(self, r, R):#, Npts=1000):
        ...
        rho_enc = self.interpolate(r, R)
        A = np.sqrt(2 * np.pi * self.rho_mean * r[:, None]**3 / 3 / rho_enc)
    ...
    def counts_to_vsf(self, rbins, Nvoids):
        ...
        for i in range(r.size - 1):
            ...
            vsf[i] = N[i] / norm
        return np.sqrt(r[:-1] * r[1:])[::-1], vsf[::-1]

    def correct_vsf(self, robs, vsf_theory, Npts=1000, checks=True, **vsf_kwargs):
        r"""
        ...
        r_sep = (4 * np.pi * self.rho_mean / 3) ** (-1/3)
        R_lims = (2 * r_sep, robs.max() * 1.1)
        ...
        if callable(vsf_theory):
            ...
            if checks: 
                ...
                if (RR.min() < r_sep):
                    raise Exception(f"VSF correction cannot be trusted at such small scales (R<{r_sep:.1f})")
